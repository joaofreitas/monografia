...
\section{Multiagent Systems Engineering - MASE}
...
Após a associação das regras, elas são estruturas em uma modelagem chamada MASE Role Model, conténdo informações de interações entre as tarefas. A imagem~\ref{fig:examplo-mase-role-model} representa a notação do Mase Role Model utilizada por~\cite{scott01}. Os retângulos são as regras, as elipses são as tarefas e as setas entre as tarefas representam as suas comunicações. Essas comunicações podem ser por meio de mensagens, caso as regras estejam separadas em agentes diferentes.
...
\begin{lstlisting}[label=code:sintaxe-concurrent-task,caption=Sintaxe do Diagrama.]
...
A imagem~\ref{fig:examplo-conversation} representa um exemplo da sintaxe do~\emph{Communication Diagram}.

\begin{figure}
	\centering
	\includegraphics[scale=0.65]{images/arvore-ontologia.png}
	\caption{Representação utilizada no~\emph{Communication Diagram}.}
